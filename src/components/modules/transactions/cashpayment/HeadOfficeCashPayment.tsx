...
const HeadOfficeCashPayment = () => {
  ...
  const handleCashPaymentSave = async () => {
    ...
    try {
      const result: any = await dispatch(storeHeadOfficeCashPayment(buildPaymentPayload(updatedTableData)) as any);
  ...
  const searchTransaction = (searchValue?: string) => {
    ...
    try {
      dispatch(
        editHeadOfficeCashPayment({ invoiceNo }, (message: string) => {
          if (message) {
            toast.error(message);
          }
        }),
