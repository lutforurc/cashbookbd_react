...
import {
  editHeadOfficeCashReceived,
  storeHeadOfficeCashReceived,
  updateHeadOfficeCashReceived,
...
const HeadOfficeCashReceived = () => {
  ...
  const initialReceivedItem: ReceivedItem = useMemo(
    () => ({
      id: '',
      mtmId: '',
      account: '',
  ...
  const buildReceivedPayload = (rows: ReceivedItem[]) => {
  ...
  const handleCashReceivedSave = async () => {
    ...
    try {
      const result: any = await dispatch(storeHeadOfficeCashReceived(buildReceivedPayload(updatedTableData)) as any);
  ...
  useEffect(() => {
    ...
    setTableData([]);
  }, [cashReceived.data, initialReceivedItem]);
  ...
  const receivedEditItem = (productId: string | number) => {
  ...
  useEffect(() => {
    if (Array.isArray(cashReceived.data)) {
      setTableData(
        cashReceived.data.map((item: any) => ({
          ...item,
      ...
      const firstItem = cashReceived.data[0];
      if (firstItem) {
        setFormData((prev) => ({
          ...prev,
  ...
  }, [cashReceived.isEdit, cashReceived.data, branchId, branchName]);
  ...
  const handleInvoiceUpdate = async () => {
    try {
      ...
      await dispatch(
        updateHeadOfficeCashReceived(buildReceivedPayload(tableData), (message: string) => {
...
export default HeadOfficeCashReceived;
