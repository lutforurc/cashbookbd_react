...
const CashPaymentPrint = React.forwardRef<HTMLDivElement, Props>(
    ({ data, fontSize = 10 }, ref) => {
        ...
        const trxMaster = data?.acc_transaction_master?.[0];
        ...
        /** ---------------------------
         * Identify Debit (Expense/Party)
         * --------------------------- */
        const debitEntry = trxDetails.find(
            (d: any) => Number(d.debit) > 0,
        );

        const creditEntry = trxDetails.find(
            (d: any) => Number(d.credit) > 0,
        );
        ...
        `}</style>
                ...
                <table className="w-full border-collapse text-xs">
                    <thead>
                        <tr>
                            ...
                            <th
                                className="border border-black px-2 py-1 text-right w-40"
                                style={{ fontSize: fs }}
                            >
                                <div className='mr-2'>
                                    Amount (Taka)
                    ...
                    <tbody>
                        <tr style={{ height: '100px' }}>
                            <td
                                className="border border-black px-2 align-middle"
                                style={{ fontSize: fs }}
                            >
                                <div className='ml-3'>
                                    {headOfAccount}
                                    {address && (
                                        <><br /><span className="text-gray-700">{address}</span></>
                                    )}
                                    {mobile && (
                                        <><br /><span className="text-gray-700">{mobile}</span></>
                                    )}
                                    {remarks && (
                                        <>
                                            <br />
                            ...
                            <td
                                className="border border-black px-2 text-right align-middle"
                                style={{ fontSize: fs }}
                            >
                                <div className='mr-2'>
                ...
                <div className="mt-10 grid grid-cols-2 gap-6 text-xs">
                    ...
                    <div className="text-right">
                        <div className="border-t border-black pt-1 w-40 ml-auto">
                            Approved By
                        </div>
                    </div>
                </div>
