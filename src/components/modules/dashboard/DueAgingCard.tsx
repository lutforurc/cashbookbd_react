...
interface DueAgingCardProps {
  ...
  isLoading?: boolean;
  /**
   * ⚠️ THE THREE WORDS BELOW ARE PROPS BECAUSE THE CARD IS NOW DRAWN TWICE.
   *
   * The trading dashboard shows receivables and payables side by side, and the
   * same four buckets mean the opposite thing on each: money owed to the branch
   * that somebody has to chase, and money the branch owes that somebody has to
   * pay. A card headed "Receivable Ageing" over the supplier book would be read
   * as money coming in, and "1,20,000 to chase" against a supplier is a wrong
   * instruction rather than a wrong label.
   *
   * All three default to the wording the shop dashboard has always used, so its
   * one call site is untouched.
   */
  title?: string;
  /** What the past-sixty-days money is, from the reader's side of the book. */
  overdueLabel?: string;
...
const DueAgingCard: React.FC<DueAgingCardProps> = ({
  aging,
  isLoading,
        ...
        {overdue > 0 && (
          <div className="flex items-center justify-between gap-2 border-t border-[rgb(var(--c-border))] bg-[rgb(var(--c-age-overdue))]/5 px-4 py-2 text-[11px]">
            <span className="font-semibold text-[rgb(var(--c-age-overdue))]">
              {thousandSeparator(Math.round(overdue))} {overdueLabel}
            </span>
