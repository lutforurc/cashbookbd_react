...
const TradingSalesImport = () => {
  ...
  return (
    <>
      ...
      {sales.isLoading ? <div className="mb-2 text-sm dark:text-[rgb(var(--c-text))]">Saving...</div> : null}
                  ...
                  <tbody className="divide-y divide-stroke dark:divide-strokedark">
                    {importedRows.length > 0 ? (
                      importedRows.map((row, index) => (
                        <tr key={`${row.slNo || index}-${row.customer}-${row.product}`} className="dark:text-[rgb(var(--c-text))]">
