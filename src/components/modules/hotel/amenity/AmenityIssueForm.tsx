...
import { FiArrowLeft, FiPlus, FiRefreshCcw, FiSave, FiTrash2 } from 'react-icons/fi';
...
const AmenityIssueForm = () => {
  ...
  /**
   * Kit lines this range owes nothing on, because enough has already gone out.
   *
   * ⚠️ Shown, not swallowed. A kit of three products that fills two lines in
   * looks like a kit that lost one, and nothing on screen told the storekeeper
  ...
  const loadFromKits = async () => {
    ...
    try {
      ...
      setSettled(Array.isArray(data.settled) ? data.settled : []);
  ...
  const save = async () => {
    ...
    try {
      ...
      reset();
      navigate(routes.hotel_amenity_issue);
    } catch (error: any) {
  ...
  return (
    <div>
      <HelmetTitle title="New Amenity Issue" />

      <div className="mb-3">
        <ButtonLoading
          onClick={() => navigate(routes.hotel_amenity_issue)}
          label="Back to issues"
          icon={<FiArrowLeft size={16} />}
        />
      </div>

      {/* What this screen IS belongs on the list behind it; what somebody
          standing here needs to know is that nothing has moved yet. */}
      <p className="mb-3 rounded border border-stroke bg-gray-50 p-2.5 text-xs leading-snug text-gray-600 dark:border-strokedark dark:bg-meta-4/40 dark:text-gray-300">
        Nothing leaves the store until you press <strong>Save issue</strong>. Quantities loaded from
        the kits are a suggestion — change any of them first.
      </p>
      ...
      <div className="mb-3 rounded border border-stroke p-3 dark:border-strokedark">
        ...
        {/* ⚠️ Why a kit of three filled only two lines in. Without this the
            storekeeper cannot tell a product that is already covered from one
            that has gone missing off the kit. */}
        {settled.length ? (
          <p className="mt-2 text-xs leading-snug text-amber-700 dark:text-amber-300">
            ...
            {settled
              .map(
                (row: any) =>
                  `${row.product_name} (${Number(row.expected)} expected, ${Number(
                    row.issued,
                  )} already issued${row.unit_name ? ' ' + row.unit_name : ''})`,
              )
              .join('; ')}
            .
          </p>
...
export default AmenityIssueForm;
