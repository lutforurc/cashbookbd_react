...
import {
  getEmployeeWiseInstallment,
  getFilterInstallment,
  installmentReceived,
...
const StaffWiseDueInstallment = (user: any) => {
  ...
  useEffect(() => {
    const responseData = installment?.employeeInstallments?.data?.data;
  ...
  }, [installment?.employeeInstallments]);
  ...
  useEffect(() => {
    ...
    dispatch(getEmployeeWiseInstallment(payload));
  }, [branchId, employeeId, startDate, endDate, dueOnly, status, upComingDays]);
  ...
  const handleSave = () => {
    ...
    dispatch(installmentReceived(payload))
      .unwrap()
      .then(() => {
        dispatch(getEmployeeWiseInstallment(payloads));
      })
      .catch((error) => {
  ...
  const handleEmployeeChange = (e: any) => setEmployeeId(e.target.value);
  ...
  return (
    <div>
      <HelmetTitle title="Employee Wise Installments" />
      <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-2 mb-3">
        <div>
          ...
          <BranchDropdown
            ...
            className="w-full font-medium text-sm p-[3px]"
...
export default StaffWiseDueInstallment;
