    ...
    <div className="text-slate-900 dark:text-white">
      ...
      <div className="mb-3 flex flex-wrap items-end gap-3">
        <div className="grid min-w-[320px] flex-1 grid-cols-1 items-end gap-3 md:grid-cols-3 xl:max-w-[75%]">
          ...
          <div><label className={labelClass}>Start Date <span className="text-red-500">*</span></label><InputDatePicker selectedDate={startDate} setSelectedDate={setStartDate} setCurrentDate={setStartDate} className="h-10 !border !border-slate-500 bg-white px-3 text-sm dark:!border-[#465367] dark:!bg-[#1c2735]" /></div>
          <div><label className={labelClass}>End Date <span className="text-red-500">*</span></label><InputDatePicker selectedDate={endDate} setSelectedDate={setEndDate} setCurrentDate={setEndDate} className="h-10 !border !border-slate-500 bg-white px-3 text-sm dark:!border-[#465367] dark:!bg-[#1c2735]" /></div>
        </div>
      ...
      {loading ? <div className="py-12"><Loader /></div> : <div className="overflow-x-auto bg-white dark:bg-[#1d2735]">
        <table className="w-full min-w-[760px] border-collapse" style={{ fontSize }}>
          <thead className="bg-slate-300 text-xs font-bold uppercase text-slate-950 dark:bg-[#3a4659] dark:text-white"><tr><th rowSpan={2} className="border border-slate-200 px-3 py-3 text-center dark:border-[#465367]">Sl. No.</th><th rowSpan={2} className="border border-slate-200 px-3 py-3 text-left dark:border-[#465367]">Account Name</th><th colSpan={2} className="border border-slate-200 px-3 py-2 text-center dark:border-[#465367]">Cash Details</th><th colSpan={2} className="border border-slate-200 px-3 py-2 text-center dark:border-[#465367]">Bank Details</th></tr><tr><th className="border border-slate-200 px-3 py-3 text-center dark:border-[#465367]">Received (Tk.)</th><th className="border border-slate-200 px-3 py-3 text-center dark:border-[#465367]">Payment (Tk.)</th><th className="border border-slate-200 px-3 py-3 text-center dark:border-[#465367]">Received (Tk.)</th><th className="border border-slate-200 px-3 py-3 text-center dark:border-[#465367]">Payment (Tk.)</th></tr></thead>
          <tbody className="divide-y divide-slate-100 dark:divide-[#2e394b]">{rows.length ? rows.map((row, index) => <tr key={`${row.name}-${index}`} className="bg-white dark:bg-[#1d2735]"><td className="px-3 py-3 text-center">{index + 1}</td><td className="px-3 py-3">{row.name || '-'}</td>{[row.cash_debit, row.cash_credit, row.bank_debit, row.bank_credit].map((amount, cell) => <td key={cell} className="px-3 py-3 text-right">{thousandSeparator(numberValue(amount))}</td>)}</tr>) : <tr><td colSpan={6} className="px-3 py-8 text-center text-slate-500">No report data found</td></tr>}</tbody>
