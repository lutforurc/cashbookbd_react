...
import { getBankBook } from './bankBookSlice';
...
const BANKBOOK_FILTER_STORAGE_KEY = 'bankbook-filter-state';

type BankBookSavedFilters = {
...
const readSavedBankBookFilters = (): BankBookSavedFilters | null => {
  if (typeof window === 'undefined') return null;

  try {
    const raw = window.sessionStorage.getItem(BANKBOOK_FILTER_STORAGE_KEY);
...
const BankBook = (user: any) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const branchDdlData = useSelector((state) => state.branchDdl);
  const bankBookData = useSelector((state) => state.bankBook);
  ...
  const runBankBook = () => {
    ...
    dispatch(getBankBook({ branchId, startDate: startD, endDate: endD }));
    saveBankBookFilters();
    setFilterOpen(false);
  };
  ...
  const saveBankBookFilters = () => {
    if (typeof window === 'undefined') return;

    const filters: BankBookSavedFilters = {
      branchId,
    ...
    window.sessionStorage.setItem(
      BANKBOOK_FILTER_STORAGE_KEY,
      JSON.stringify(filters),
    );
  };

  useEffect(() => {
    const savedFilters = readSavedBankBookFilters();
  ...
  useEffect(() => {
    setTableData(bankBookData?.data);
  }, [bankBookData]);
  ...
  const columns = [
      ...
      render: (row: any) => (
        <div className="w-full max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-3xl xl:max-w-4xl">
          <div className="truncate">
	            <a href={routes.report_ledger} target="_blank">
    ...
    {
      key: 'from_bank',
      header: 'Transaction Bank',
      width: '160px',
      render: (row: any) => (
        <div className="break-words whitespace-normal">
          {row?.from_bank ? row.from_bank : '-'}
        </div>
      ),
    },
    {
