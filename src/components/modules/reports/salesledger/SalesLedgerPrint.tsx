...
const SalesLedgerPrint = forwardRef<HTMLDivElement, Props>(
  (
    ...
    const ledgerCalc = useMemo(() => new SalesLedgerCalculator(rowsArr || []), [rowsArr]);
    ...
    return (
      <div ref={ref} className="p-8 text-sm text-gray-900 print-root">
                ...
                <table className="w-full table-fixed border-collapse">
                  ...
                  <tbody>
                    {pageRows.length ? (
                      pageRows.map((row: any, idx: number) => {
                        ...
                        return (
                          <tr key={row?.id ?? `${pIdx}-${idx}`} className="avoid-break align-top">
                            ...
                            <td style={{ fontSize: fs }} className="border border-gray-900 px-2 py-1 text-right">
                              {receivedValue ? thousandSeparator(receivedValue, 0) : "-"}
                            </td>
                      ...
                      <tr>
                        <td colSpan={10} className="border border-gray-900 px-3 py-6 text-center text-gray-500">
                          No data found
                        </td>
                      </tr>
                    )}
        ...
        <div className="mt-2 text-xs text-gray-900">
          * This document is system generated. Printed: {dayjs().format("DD-MMM-YYYY hh:mm A")}
        </div>
      </div>
    );
  }
