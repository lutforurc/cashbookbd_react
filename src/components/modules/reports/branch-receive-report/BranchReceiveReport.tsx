...
import { API_REPORT_BRANCH_RECEIVE_URL } from '../../../services/apiRoutes';
...
const BranchReceiveReport = ({ user }: any) => {
  ...
  const runReport = async () => {
    ...
    try {
      const response = await httpService.post(API_REPORT_BRANCH_RECEIVE_URL, {
      ...
      setRows(list);
      if (list.length === 0) {
        setError('No branch receive found for this branch and date range.');
      }
    } catch (err: any) {
      setRows([]);
    ...
    () =>
      rows.reduce(
        (acc, r) => {
          ...
          acc.damaged += Number(r.damaged) || 0;
          acc.shortage += Number(r.shortage) || 0;
          acc.balance += Number(r.balance) || 0;
          return acc;
        },
        { opening: 0, received: 0, damaged: 0, shortage: 0, balance: 0 },
      ),
    [rows],
  );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cellClass: 'text-right',
        render: (r: ReportRow) => (
          <span className="font-semibold text-success">{thousandSeparator(Number(r.received))}</span>
        ),
      },
      {
        key: 'damaged',
        ...
        cellClass: 'text-right',
        render: (r: ReportRow) => (
          <span className={Number(r.damaged) > 0 ? 'font-semibold text-danger' : ''}>
            {thousandSeparator(Number(r.damaged))}
          </span>
        ),
      },
      {
        ...
        cellClass: 'text-right',
        render: (r: ReportRow) => (
          <span className={Number(r.shortage) > 0 ? 'font-semibold text-warning' : ''}>
            {thousandSeparator(Number(r.shortage))}
          </span>
        ),
      },
      {
  ...
  const footerRows = useMemo(() => {
    ...
    return [
      [
        ...
        { label: thousandSeparator(totals.received), className: 'text-right' },
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr>
                ...
                <th className="border border-black px-2 py-1 text-right w-28">Received</th>
                <th className="border border-black px-2 py-1 text-right w-28">Damaged</th>
                <th className="border border-black px-2 py-1 text-right w-28">Short</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.product_id}>
                  ...
                  <td className="border border-black px-2 py-1 text-right">{thousandSeparator(Number(r.received))}</td>
                  <td className="border border-black px-2 py-1 text-right">{thousandSeparator(Number(r.damaged))}</td>
                  <td className="border border-black px-2 py-1 text-right">{thousandSeparator(Number(r.shortage))}</td>
            ...
            <tfoot>
              <tr className="font-semibold">
                ...
                <td className="border border-black px-2 py-1 text-right">{thousandSeparator(totals.received)}</td>
...
export default BranchReceiveReport;
