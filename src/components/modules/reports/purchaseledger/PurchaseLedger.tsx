...
const readSavedPurchaseLedgerFilters = (): PurchaseLedgerSavedFilters | null => {
  if (typeof window === 'undefined') return null;

  try {
    const raw = window.sessionStorage.getItem(PURCHASE_LEDGER_FILTER_STORAGE_KEY);
...
const PurchaseLedger = (user: any) => {
  ...
  const savePurchaseLedgerFilters = () => {
    if (typeof window === 'undefined') return;

    const filters: PurchaseLedgerSavedFilters = {
      branchId,
      ledgerId,
      productId,
    ...
    window.sessionStorage.setItem(
      PURCHASE_LEDGER_FILTER_STORAGE_KEY,
      JSON.stringify(filters),
    );
  };

  useEffect(() => {
    const savedFilters = readSavedPurchaseLedgerFilters();
        ...
        return (
          <div className="text-left">
            <span className='block'>{ formatTransportationNumber(row?.purchase_master?.vehicle_no)}</span>
             <span className='text-green-500 block'>{ row?.purchase_master?.purchase_order?.order_number}</span>
             { row?.purchase_master?.purchase_order?.delivery_location && (
               <span className='text-green-500 block'>{ row?.purchase_master?.purchase_order?.delivery_location}</span>
             )}
          </div>
        );
      },
