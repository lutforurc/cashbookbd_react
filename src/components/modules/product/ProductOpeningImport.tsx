...
import {
  API_PRODUCT_OPENING_IMPORT_CHECK_URL,
  API_PRODUCT_OPENING_IMPORT_ROW_URL,
...
/**
 * Opening stock from an Excel sheet.
 *
 * The sheet is loaded into a grid where any cell can be corrected. Check asks
 * the server what every row comes to -- a new product, an existing one whose
 * prices change, or a refusal -- and Ok saves the rows one call at a time, so
 * a long sheet never meets the request time limit and a refused row stays on
 * screen, red, to be fixed and sent again.
 *
 * The server does the matching and the saving (ItemController::
 * resolveOpeningImportRow): each product gets its own opening voucher, the
 * same kind the product list raises, so any one of them can be corrected or
 * deleted there afterwards.
 */
...
const FIELDS: { key: Field; header: string; className: string; numeric?: boolean }[] = [
  { key: 'brand_name', header: 'Brand', className: 'min-w-24' },
  { key: 'group', header: 'Group', className: 'min-w-24' },
  { key: 'category', header: 'Category', className: 'min-w-24' },
  { key: 'code', header: 'Code', className: 'min-w-24' },
  { key: 'product_name', header: 'Product Name', className: 'min-w-56' },
  { key: 'unit', header: 'Unit', className: 'min-w-16' },
  { key: 'purchase_price', header: 'P. Price', className: 'min-w-20 text-right', numeric: true },
  { key: 'sales_price', header: 'Sales Price', className: 'min-w-20 text-right', numeric: true },
  { key: 'stock', header: 'Stock', className: 'min-w-16 text-right', numeric: true },
];

/**
 * Every heading a sheet may carry for a column, compared with case, spaces,
 * underscores and full stops taken out -- so "brand_name", "Brand Name" and
 * "BRAND NAME" are one heading, and the older "P. Price" still finds its way.
 */
const HEADING_ALIASES: Record<Field, string[]> = {
  brand_name: ['brandname', 'brand'],
  group: ['group', 'groupname', 'productgroup'],
  category: ['category', 'categoryname'],
  code: ['code', 'productcode', 'itemcode'],
  product_name: ['productname', 'name', 'product', 'itemname'],
  unit: ['unit', 'unitname'],
  purchase_price: ['purchaseprice', 'pprice', 'purchase', 'purchaserate'],
  sales_price: ['salesprice', 'saleprice', 'sprice', 'sales', 'salesrate'],
  stock: ['stock', 'qty', 'quantity', 'openingstock'],
};
...
const rowsFromWorkbook = (workbook: XLSX.WorkBook): SheetRow[] | null => {
  ...
  if (columnOf('product_name') < 0) return null;
...
const ProductOpeningImport = () => {
  ...
  // can change another's verdict (two rows that shared a code no longer do).
  const [stale, setStale] = useState(false);

  // The same branch switch the product list and Add Product read before they
  // offer an opening; the server refuses without it too.
  const openingOn = isBranchSettingOn(settings, 'is_opening');
  const productTypes = category?.ddlData?.data?.product_type;

  useEffect(() => {
    dispatch(getCategoryDdl());
  }, []);

  // Stock bought to sell is Inventory. The list comes sorted by name, so taking
  // its first entry, as Add Product does, would make every product an Asset.
  useEffect(() => {
    if (productType || !Array.isArray(productTypes) || productTypes.length === 0) return;
    const inventory = productTypes.find((type: any) => String(type?.name).toLowerCase() === 'inventory');
    setProductType(String((inventory ?? productTypes[0]).id));
  ...
  // raw keeps pasted text as text, so a code like 00123 keeps its noughts.
  const handlePaste = () => {
    if (!pasteText.trim()) {
  ...
  const downloadFormat = () => {
    const worksheet = XLSX.utils.aoa_to_sheet([SHEET_HEADINGS]);
    worksheet['!cols'] = [8, 16, 14, 14, 14, 36, 10, 14, 14, 10].map((wch) => ({ wch }));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Product Opening');
    XLSX.writeFile(workbook, 'product-opening-import-format.xlsx');
  };
  ...
  const runCheck = async () => {
    ...
    try {
      ...
      // number in the grid's # column and a code already saved above counts
      // as a repeat.
      const res = await httpService.post(API_PRODUCT_OPENING_IMPORT_CHECK_URL, {
  ...
  const runSave = async () => {
    ...
    for (const [index, row] of queue.entries()) {
      ...
      try {
        const res = await httpService.post(API_PRODUCT_OPENING_IMPORT_ROW_URL, {
          row: row.data,
    ...
    if (failed) {
      ...
      toast.success(`${queue.length} product(s) saved.`);
    }
  };
  ...
  const canSave =
    ...
    !stale &&
    productType !== '' &&
    pending.every((row) => row.check && row.check.status !== 'error');

  const count = (status: CheckResult['status']) =>
    pending.filter((row) => row.check?.status === status).length;
  const openingValue = rows.reduce(
    (sum, row) => sum + toNumber(row.data.stock) * toNumber(row.data.purchase_price),
    0,
  );
      ...
      <div>
        <span className={row.check.status === 'new' ? 'text-blue-600 dark:text-blue-400' : 'text-green-600'}>
          {row.check.status === 'new'
            ? 'New product'
            : `Existing: ${row.check.product_name ?? ''} — prices update`}
        </span>
  ...
  return (
    <>
      <HelmetTitle title="Product Opening Import" />
      ...
      {!openingOn && (
        <div className="mb-3 rounded border border-amber-400 bg-amber-50 p-3 text-sm text-amber-800 dark:bg-transparent dark:text-amber-300">
          Opening stock is switched off for this branch. Turn it on in the branch settings to import.
        </div>
      )}
      ...
      <div className="rounded-sm p-4 shadow-default">
        <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
          <div className="w-56">
            <DropdownCommon
              id="product_type"
              name="product_type"
              label="Product Type (for new products)"
        ...
        {rows.length > 0 && (
          <div className="mb-2 flex flex-wrap gap-x-4 gap-y-1 text-sm dark:text-[rgb(var(--c-text))]">
            ...
            <span>Opening value: {thousandSeparator(openingValue) || '0'}</span>
          </div>
        )}
...
export default ProductOpeningImport;
