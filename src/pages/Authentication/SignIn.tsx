...
const SignIn: React.FC = () => {
  const { isLoading, errors, isLoggedIn } = useSelector(
  ...
  const getTitle = () => {
    if (hostname === "accounts.nibirnirman.cashbookbd.com") {
      return "Sign In to Nibir Nirman";
    } else if (hostname === "accounts.sinthia.cashbookbd.com") {
      return "Sign In to Sinthia Electronics";
    } else {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      let { from } = prevLocation.state || {
        from: { pathname: ROUTES.dashboard },
      };
      navigate(from, { replace: true });
      dispatch(getSettings());
    }
  }, [isLoggedIn, prevLocation, navigate]);
  ...
  const handleLogin = (e: any) => {
    ...
    if (!formData.email || !formData.password) {
      dispatch({
        ...
        payload: 'Please add the required info.',
      });
      toast.error('Please add the required info.');
      return;
    }
      ...
      <div className="rounded-sm border h-screen border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="flex flex-wrap justify-center items-center">
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full h-screen p-4 sm:p-12.5 xl:p-17.5 flex flex-col justify-center align-middle">
              ...
              {process.env.NODE_ENV === 'development' ? (
                <button className="mb-9" onClick={handleSetUser}>
                  Set User
                </button>
              ) : (
                ''
